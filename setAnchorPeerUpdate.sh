...
set -x
peer channel fetch config config_block.pb -o localhost:9999 --ordererTLSHostnameOverride orderer.center.com -c $CHANNEL_NAME --tls --cafile "$ORDERER_CA"
{ set +x; } 2>/dev/null
...
peer channel update -f Org1MSPAnchor.tx -c "${CHANNEL_NAME}" -o localhost:9999  --ordererTLSHostnameOverride orderer.center.com --tls --cafile "$ORDERER_CA"
popd
...
peer channel update -f Org2MSPAnchor.tx -c "${CHANNEL_NAME}" -o localhost:9999  --ordererTLSHostnameOverride orderer.center.com --tls --cafile "$ORDERER_CA"
popd